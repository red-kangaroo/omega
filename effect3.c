...
void polymorph (int blessing)
{
	...
	if (m_immunityp (m, OTHER_MAGIC) || (m->level > random_range (12))) {
		...
		while ((newmonster == ML0 + 4) || (newmonster == ML7 + 3) || (Monsters[newmonster].level <= m->level) || (Monsters[newmonster].uniqueness != COMMON));
	    } else {
		do
		    newmonster = random_range (NUMMONSTERS);
		while ((newmonster == ML0 + 4) || (newmonster == ML7 + 3) || (Monsters[newmonster].uniqueness != COMMON));
	    }
